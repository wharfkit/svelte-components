<script lang="ts">
	import { Cluster, Stack, Button, Label, Card, Select } from '$lib';
...
<Stack tag="section" id="select">
	<h2 class="text-headline">Select</h2>

	<Card title="Pill Select">
		<Stack class="items-start">
			<Stack class="gap-2">
				<Label for="date-range">Select a date range</Label>
				<Select id="date-range" {options} bind:selected />
			</Stack>
			<strong>Value in parent: {selected.label}</strong>
			<Button onclick={() => (selected = options[2])}>Test external control</Button>
		</Stack>
	</Card>
	...
	<Card title="Form Select">
		<Cluster>
			<Stack class="gap-2">
				<Label for="form-select">Default</Label>
				<Select
					variant="form"
					id="form-select"
					options={formOptions}
				...
				/>
				<strong>Value in parent: {selectedFormOption.label}</strong>
			</Stack>
